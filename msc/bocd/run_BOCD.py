...
def get_BOCD_changepoints(series, verbose=True):
    ...
    changepoints = []
    for x in tqdm(series[:DELAY], desc="pre", miniters=int(len(series)/100)):
        bocd.update(x)
    for x in tqdm(series[DELAY:], desc="main", miniters=int(len(series)/100)):
        bocd.update(x)
...
def plot_BOCD_changepoints(data, changepoints, sfreq, output_path: Optional[str] = None):
    sns.lineplot(data=data, label=f"data")
    ...
    if output_path:
        plt.savefig(os.path.join(output_path, f"data_with_change_points.png"))
    else:
        plt.show()
    plt.clf()
...
def plot_BOCD_changepoints_and_seizure(data, changepoints, sfreq, onset, offset, output_path: Optional[str] = None,
    ...
    sns.scatterplot(x=changepoints, y=[0] * len(changepoints), color='orange', zorder=3, label="BOCD changepoints")
        # add onset, offset
    plt.scatter(onset, 0, c='r', label='seizure onset', zorder=4)
    plt.scatter(offset, 0, c='r', label='seizure offset', zorder=4)
    plt.title("Bayesian Online Changepoint Detection on EEG")
    plt.xlabel(f"sample number; sfreq={sfreq}")
