...
from msc.data_utils import (
    count_nans,
    get_dataset,
    get_event_sample_times,
    get_sample_times,
)
...
class parallel_embeddor:
    ...
    def __init__(self, config, ds, mode: str):
        assert mode in (
            "online",
            "offline",
        ), f"error: {mode=} should be 'online' or 'offline'"
        self.config = config
        self.ds = ds
        self.mode = mode
    ...
    def compute_ds_stats(self, N):
        ...
        for idx, t in enumerate(sample_times):  # type: ignore
            data_t = self.ds.get_data(
                t, self.config["duration"], np.arange(self.config["num_channels"])
            )
            samples.append(data_t)
            nan_count = count_nans(data_t)
            if nan_count != 0:
                self.logger.info(
                    f"At time {t} ({idx}/{len(sample_times)}) {nan_report(data_t)}"
                )
                segments_with_nan += 1
